...
class KeystoreUtil: KeystoreUtilProtocol {
    ...
    static func encode(privateKey: Data, password: String, salt: Data, iv: Data) throws -> Data {
        ...
        let mac = concat.xdc3.keccak256
        
        // create keystore
        let crypto = KeystoreFileCrypto(
            cipher: "aes-128-ctr",
            cipherparams: KeystoreFileCryptoCipherParams(iv: iv.xdc3.hexString.xdc3.noHexPrefix),
            ciphertext: ciphertext.xdc3.hexString.xdc3.noHexPrefix,
            kdf: keyDerivator.algorithm.function(),
            kdfparams: KeystoreFileCryptoKdfParams(c: self.dkround, dklen: self.dklen, prf: keyDerivator.algorithm.hash(), salt: salt.xdc3.hexString.xdc3.noHexPrefix),
            mac: mac.xdc3.hexString.xdc3.noHexPrefix)
    ...
    static func decode(data: Data, password: String) throws -> Data {
        ...
        let concat = macKey + ciphertext
        let mac = concat.xdc3.keccak256
        guard mac.xdc3.hexString.xdc3.noHexPrefix == keystore.crypto.mac else {
            throw KeystoreUtilError.corruptedKeystore
        }
        
        // decrypt ciphertext with encryption key
        let encKey = derivedKey.subdata(in: 0..<16)
        let iv = keystore.crypto.cipherparams.iv.xdc3.hexData
