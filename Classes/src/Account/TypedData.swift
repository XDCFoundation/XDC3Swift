...
extension TypedData {
    public var typeHash: Data { encodeType(primaryType: primaryType).xdc3.keccak256 }

    // Whole data blob hash to sign
    public func signableHash() throws -> Data {
        var data = Data([0x19, 0x01])
        data.append(try encodeData(data: domain, type: "EIP712Domain").xdc3.keccak256)
        data.append(try encodeData(data: message, type: primaryType).xdc3.keccak256)
    ...
    public func encodeData(data: JSON, type: String) throws -> Data {
        var encoded = try ABIEncoder.encode(encodeType(primaryType: type).xdc3.keccak256, staticSize: 32).bytes
        ...
        let recursiveEncoded: [UInt8] = try valueTypes.flatMap { variable -> [UInt8] in
            if types[variable.type] != nil {
                ...
                return try encodeData(data: json, type: variable.type).xdc3.keccak256.xdc3.bytes
    ...
    private func parseAtomicType(_ data: JSON, type: String) throws -> [UInt8] {
        ...
        switch abiType {
        case .DynamicString:
            guard let value = data.stringValue?.xdc3.keccak256 else {
                throw ABIError.invalidValue
            }
            return try ABIEncoder.encodeRaw(value, forType: .FixedBytes(32)).bytes
        case .DynamicBytes:
            guard let value = data.stringValue.flatMap(Data.init(hex:))?.xdc3.keccak256 else {
                throw ABIError.invalidValue
            }
            return try ABIEncoder.encodeRaw(value, forType: .FixedBytes(32)).bytes
        ...
        case .DynamicArray(let nested):
            ...
            return Data(encoded).xdc3.keccak256.xdc3.bytes
