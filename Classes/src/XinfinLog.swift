...
public struct XinfinLog: Equatable {
...
extension XinfinLog: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.address = try values.decode(XinfinAddress.self, forKey: .address)
        ...
        if let blockNumberString = try? values.decode(String.self, forKey: .blockNumber) {
            self.blockNumber = XinfinBlock(rawValue: blockNumberString)
        } else {
            self.blockNumber = XinfinBlock.Earliest
        }
    }
...
extension XinfinLog: Comparable {
    public static func < (lhs: XinfinLog, rhs: XinfinLog) -> Bool {
