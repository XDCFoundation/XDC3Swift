    ...
    public static func encode(_ value: ABIType,
        ...
        case let data as Data:
            if let staticSize = staticSize {
                return try ABIEncoder.encodeRaw(String(bytes: data.xdc3.bytes), forType: .FixedBytes(staticSize), padded: !packed)
            } else {
                return try ABIEncoder.encodeRaw(String(bytes: data.xdc3.bytes), forType: type, padded: !packed)
            }
