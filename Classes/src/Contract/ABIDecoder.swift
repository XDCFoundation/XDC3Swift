...
public class ABIDecoder {
    ...
    static func decodeData(_ data: RawABI, types: [ABIRawType], asArray: Bool = false) throws -> RawParsedABI {
        ...
        for type in types {
            if data == "0x" && type.isArray {
                ...
                guard let bytes = data.xdc3.bytesFromHex else { throw ABIError.invalidValue }
    ...
    static func decode(_ data: [UInt8], forType type: ABIRawType, offset: Int = 0) throws -> ABIEntry {
        ...
        case .DynamicBytes:
            ...
            guard let sizeHex = (try decode(data, forType: ABIRawType.FixedUInt(256), offset: newOffset)).first, let bint = BigInt(hex: sizeHex.xdc3.noHexPrefix) else {
                throw ABIError.invalidValue
            }
            let size = Int(bint)
            guard size > 0 else {
        ...
        case .FixedInt(_):
            ...
            return [String(hexFromBytes: bint.xdc3.bytes)]
        case .FixedUInt(_):
            guard data.count > 0 else {
                return [""]
            }
