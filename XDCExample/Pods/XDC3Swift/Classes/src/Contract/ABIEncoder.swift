...
public class ABIEncoder {
    ...
    static func encodeRaw(_ value: Data,
        ...
        return try encodeRaw(value.xdc3.hexString, forType: type, padded: padded, size: size)
    }
    ...
    private static func encodeRaw(_ value: String,
        ...
        case .FixedUInt(let typeSize):
            ...
            guard let int = value.xdc3.isNumeric ? BigUInt(value) : BigUInt(hex: value) else {
                throw ABIError.invalidValue
            }
            let bytes = int.xdc3.bytes // should be <= 32 bytes
        ...
        case .FixedInt(_):
            guard let int = value.xdc3.isNumeric ? BigInt(value) : BigInt(hex: value) else {
        ...
        case .FixedAddress:
         //   print(value.xdc3)
            guard let bytes = value.xdc3.bytesFromHex else { throw ABIError.invalidValue } // Must be 20 bytes
            if padded  {
                encoded = [UInt8](repeating: 0x00, count: 32 - bytes.count) + bytes
            } else {
                encoded = bytes
        ...
        case .DynamicBytes:
            ...
            guard let bytes = value.xdc3.bytesFromHex else { throw ABIError.invalidValue }
            let len = try encodeRaw(String(bytes.count), forType: ABIRawType.FixedUInt(256)).bytes
            let pack: Int
            if bytes.count == 0 {
                pack = 0
        ...
        case .DynamicArray(let type):
            ...
            let stringValue = value.xdc3.noHexPrefix //noHexPrefix
